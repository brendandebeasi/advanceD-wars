        ...
        EntityPointer = ig.Entity.extend({
            ...
            check: function(other) {
                if( this.isClicking && typeof(other.clicked) == 'function') {
                    ...
                    if(this.hasUnitSelected) this.destroyActionMarkers();
                    else this.hasUnitSelected = true;

                    other.clicked();
                    if(other.active) {
                        var movementBounds = this.calculatePossibleMovements(other);
                    }
                }
                else if(this.isClicking) this.destroyActionMarkers();
            },
            destroyActionMarkers: function() {
                var moveMarkers = ig.game.getEntitiesByType(EntityActionMove);
                var attackMarkers = ig.game.getEntitiesByType(EntityActionAttack);
